												...
												<div class="media form-group" id="step">
													...
													<div class="media-left media-middle">
														...
														background-repeat:no-repeat; background-size:contain">
															<span class='glyphicon glyphicon-remove-sign' id='delImgBtn' aria-hidden='true' style="display:block"></span>
															<input type="hidden" id="ino" name="ilist[0].ino" value="${imageVO.ino}">
															<div id="noImg">
...
<!-- 															<input type='hidden' id='ino' name='ilist[0].ino'> -->
												...
												<input type='hidden' id='sbno' name='ilist[0].bno' value="${imageVO.bno}">
					...
					<div class="right row">
						...
						<div class="form-group col-md-4">
							...
							<a href="#" onclick='history.back(); return false;'><input type="button" class="btn btn-default-right" value="취 소"></a>
						</div>
					</div>
			</div>
		</div>
	...
	$(document).ready(function() {
		...
		function indexOfStep() {
			...
			$child.find('input#ino').attr('name', 'ilist[' + i + '].ino');
		...
		$(document).on("click", "#delStepBtn", function(event) {
			...
			var $this = $(this);
			var $thisDiv = $this.parent().parent(".media");
			var ino = $thisDiv.find("#ino").val();
			var bno = $("#bno").val();
			var step = $thisDiv.find("#stepCIndex").val();
			console.log("ino : " + ino);
			console.log("bno : " + bno);
			...
			if(!confirm("완전히 삭제됩니다. 정말 삭제하시겠습니까?")){
				return;
			}else{
			$thisDiv.remove();
			stepCnt--;
			...
			$.ajax({
				url : '/recipe/stepDelete',
				data : {bno : bno, ino : ino, step : step},
				dataType : 'json',
				method : 'POST',
				success : function() {
...
/* 		$("#regBtn").submit(function(event) {
		...
		$(".uploadedList .regBtn").each(function(index) {
